...
def modify_server_list(serverSocket, address, request):
	...
	if (request[:3] == 'ADD'):
		...
		serverList.append([address[0],actualPort]) #Add server
	elif (request[:3] == 'DEL'):
		print ('Deleting: ' + str(address[0]) + ', ' + str(actualPort))
		serverList.remove([address[0],actualPort]) #Remove/close server
		return
	else:
...
def try_primary_server(clientRequest, address):
	...
	while(1):
		...
		primarySocket = create_socket('localhost',primaryServer[1])
...
def process_primary_response(primarySocket, ip, address):
	...
	if response[:3] == 'ERR': # Process completed -> Failed
		...
		serverList = pickle.load( open('replica_port_db.p','rb'))
		#Reestablish connection
		primarySocket = create_socket(ip,address)
		primarySocket.send(('_'.join(serverList)).encode('utf-8'))
